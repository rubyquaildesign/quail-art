...
fn curl(p: vec2<f32>) -> vec2<f32> {
    ...
    let dy: vec2<f32> = vec2<f32>(0., e);
    let p_x0: vec2<f32> = snoise2(p - dx);
    let p_x1: vec2<f32> = snoise2(p + dx);
    let p_y0: vec2<f32> = snoise2(p - dy);
    let p_y1: vec2<f32> = snoise2(p + dy);
    let x: f32 = p_x1.y + p_x0.y;
    let y: f32 = p_y1.x - p_y0.x;
    let divisor: f32 = 1. / (2. * e);
    return vec2<f32>(x, y) * divisor;
} 